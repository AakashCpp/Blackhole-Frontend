...
const SecurityReportDashboard = ({ SCAN_DATA }) => {
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(18);
    doc.text("BlackHole Security Scan Report", 14, 13);
    ...
    doc.text(
      `Scan Date: ${SCAN_DATA.scanDate || new Date().toLocaleString()}`,
      14,
      36,
    );
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ["Resolved Issues", resolvedIds.length],
        [
          "Remediation Progress",
          `${Math.round((resolvedIds.length / stats.total) * 100) || 0}%`,
        ],
      ],
      styles: { fontSize: 10 },
      headStyles: {
    ...
    doc.setFontSize(14);
    doc.text("Detailed Vulnerabilities", 14, doc.lastAutoTable.finalY + 10);
    ...
    SCAN_DATA.vulnerabilities.forEach((vuln) => {
      const status = resolvedIds.includes(vuln.id) ? "FIXED" : "OPEN";
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(`Generated by BlackHole Security Scanner`, 14, 290);

      doc.text(`Page ${i} of ${pageCount}`, 180, 290);
    }

    doc.save("blackhole_security_report.pdf");
  };
